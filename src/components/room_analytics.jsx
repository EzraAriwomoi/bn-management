...
export function RoomAnalytics() {
  ...
  const metricsData = rooms.map(room => {
    ...
    return {
      ...
      revenue: metric?.revenue || 0, // <-- now matches per-day revenue from backend
      totalDays: metric?.total_days_in_period || getTotalDaysInPeriod(),
    };
  });
  ...
  const vacantRoomsToday = period === "today" ? metricsData.filter(r => r.occupiedDays === 0).map(r => r.roomName) : [];

  // -------------------- OUTSTANDING --------------------
  const outstandingToday = bookings
    .filter(b => new Date(b.check_in).toDateString() === today.toDateString() && Number(b.balance) > 0)
    .reduce((sum, b) => sum + Number(b.balance || 0), 0);

  const totalOutstandingPrevious = bookings
    .filter(b => new Date(b.check_in) < today && Number(b.balance) > 0)
    .reduce((sum, b) => sum + Number(b.balance || 0), 0);

  const totalOutstanding = outstandingToday + totalOutstandingPrevious;

  // -------------------- CHART DATA --------------------
  const totalRevenue = metricsData.reduce((sum, r) => sum + r.revenue, 0); // <-- uses backend per-day revenue
  ...
  const roomColors = ["#f8ee5e","#663711","#7e7e7e","#4BC0C0","#3c158a","#000000","#d8b92f"];
  ...
  const revenueYAxisMax = period==="today" ? 6000 : period==="week" ? 18000 : 80000;
  ...
  return (
    <div className="room-analytics">
        ...
        <CardContent>
          <div className="period-buttons">
            {["today","week","month","lastMonth"].map(p => (
              <Button key={p} variant="outline" onClick={()=>setPeriod(p)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm text-muted-foreground">Rooms Occupied</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            {period==="today" && (
              <div className="mt-2 text-xs text-muted-foreground space-y-1">
                {vacantRoomsToday.length ? vacantRoomsToday.map((r,i)=><div key={r}>{i+1}. {r}</div>) : <div>All rooms occupied</div>}
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="kpi-subtext">{period==="lastMonth"?"Last month":period==="today"?"Today":`This ${period}`}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold text-black">KES {totalOutstanding.toLocaleString()}</p>
            <div className="mt-4 space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Outstanding Today</span>
                <span className="text-red-500 font-medium">KES {outstandingToday.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Previous Outstanding</span>
                <span className="text-orange-500 font-medium">KES {totalOutstandingPrevious.toLocaleString()}</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="charts-grid">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Days occupied out of total days in period</CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ...
              <div className="text-xs text-muted-foreground mt-1">{getPeriodRangeLabel()}</div>
            </CardDescription>
          </CardHeader>
          <CardContent className="chart-wrapper">
            <ResponsiveContainer width="100%" height={300}>
      ...
      <Card>
        ...
        <CardContent className="chart-wrapper h-[400px]">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={pieData} dataKey="value" cx="50%" cy="50%" outerRadius={120}>
                {pieData.map((_,i)=><Cell key={i} fill={roomColors[i % roomColors.length]} />)}
              </Pie>
              <Tooltip formatter={value=>`${value} days`} />
            </PieChart>
          </ResponsiveContainer>
